...
async function fetchDataRBTS() { 
    try {
        ...
        console.log("Robot data received:", data.Y_ProcesosAuto);
        ...
        cards.forEach((card) => {
            ...
            if (data.Y_ProcesosAuto && Array.isArray(data.Y_ProcesosAuto)) {
                let robotCodes;
                ...
                data.Y_ProcesosAuto.forEach(robot => {
                    console.log(`Checking robot ${robot.Codigo} with status ${robot.Estado} for card ${title}`);
                    
                    // Check if this robot belongs to this card and is not in "Ok" status
                    // Note: we're checking for "Demorado" or any status that's not "Ok"
                    if (robotCodes && robotCodes.includes(robot.Codigo) && robot.Estado !== 'Ok') {
                        console.log(`Found delayed robot: ${robot.Codigo} with status: ${robot.Estado}`);
                        delayedCount++;
